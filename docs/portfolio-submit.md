...
현재는 재고 예약 실패나 결제 실패가 발생하면 주문 상태를 `PAYMENT_FAILED`로 반영하고, 사용자가 그 상태에서 재결제를 하거나 취소할 수 있도록 구성했습니다. 또한 장시간 결제가 완료되지 않은 주문은 `PAYMENT_EXPIRED`로 전이한 뒤 다시 결제를 시도할 수 있게 했습니다.
...
처음에는 `local`, `docker`, `saga-e2e` 프로필 조합 위에 ECS 환경변수를 계속 덧붙이는 방식으로 접근했습니다. 이후 `local`, `aws` 두 케이스로 단순화하고, 로컬 기능은 `local` 프로필 기본값으로 통합했습니다.  
