...
ALTER TABLE refresh_token ALTER COLUMN refresh_token_id SET DEFAULT nextval('refresh_token_refresh_token_id_seq'::regclass);;
...
CREATE INDEX idx_refresh_token_user_active ON refresh_token USING btree (user_id) WHERE (revoked = false);;

CREATE INDEX idx_refresh_token_user_id ON refresh_token USING btree (user_id);;

ALTER TABLE refresh_token
    ADD CONSTRAINT fk_refresh_token_replace FOREIGN KEY (replaced_by_token_id) REFERENCES refresh_token(token_id);;
...
COMMENT ON TABLE refresh_token IS '테이블: refresh_token';
COMMENT ON COLUMN refresh_token.refresh_token_id IS 'refresh token id';
COMMENT ON COLUMN refresh_token.token_id IS 'token id';
COMMENT ON COLUMN refresh_token.token_hash IS 'token hash';
COMMENT ON COLUMN refresh_token.user_id IS 'user id';
COMMENT ON COLUMN refresh_token.expires_at IS 'expires at';
COMMENT ON COLUMN refresh_token.created_at IS 'created at';
COMMENT ON COLUMN refresh_token.last_used_at IS 'last used at';
COMMENT ON COLUMN refresh_token.revoked IS 'revoked';
COMMENT ON COLUMN refresh_token.revoked_at IS 'revoked at';
COMMENT ON COLUMN refresh_token.replaced_by_token_id IS 'replaced by token id';
COMMENT ON COLUMN refresh_token.updated_at IS 'updated at';
COMMENT ON COLUMN refresh_token.created_by IS 'created by';
COMMENT ON COLUMN refresh_token.updated_by IS 'updated by';
