    ...
    <div class="auth-card">
      <div class="auth-card__form">
        <div class="auth-card__form-header">
          <h2>로그인</h2>
          <p class="muted">아이디와 비밀번호를 입력해 주세요.</p>
        </div>
        ...
        <p v-if="notice" class="auth-info">{{ notice }}</p>
...
const error = ref('');
const notice = ref(route.query.registered === '1' ? '회원가입이 완료되었습니다. 로그인해 주세요.' : '');
...
async function submit() {
  ...
  try {
    ...
    error.value = parsed.message || '로그인에 실패했습니다.';
  }
}
</script>
